    ...
    def run_episode(self, scenario: AttackScenario, verbose: bool = True) -> Dict[str, Any]:
        ...
        def query_print_callback(query: str, result: Optional[str]) -> None:
            ...
            if result is None:
                ...
                print(
                    colorize("Watson queried", Colors.QUERY + Colors.BOLD)
                    + f': "{colorize(query, Colors.QUERY)}"'
                )
            else:
                ...
                print(
                    ...
                    + f': "{colorize(result_preview, Colors.RESPONSE)}"'
                )
                ...
                if incremental_rewards and len(incremental_rewards) > 0:
                    ...
                    total_color = (
                        ...
                        if total_incremental_reward > 0
                        else (
                            Colors.REWARD_NEGATIVE
                            if total_incremental_reward < 0
                            else Colors.REWARD_NEUTRAL
                        )
                    )
                    print(
                        f"Total reward so far: {colorize(f'{total_incremental_reward:.2f}', total_color + Colors.BOLD)}\n"
                    )
                else:
        ...
        if verbose:
            ...
            print(
                f"Incremental reward total: {colorize(f'{total_incremental_reward:.2f}', inc_color + Colors.BOLD)}\n"
            )
