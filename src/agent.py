    ...
    def investigate(
        ...
        while self.tool_calls_made < self.max_tool_calls:
            ...
            message_dict = {
                ...
                or None,  # Handle None content when tool calls are present
            }
            ...
            if message.tool_calls:
                                ...
                                if verbose:
                                    if query_print_callback:
                                        try:
                                            ...
                                            preview = query_result[:200] + (
                                                "..." if len(query_result) > 200 else ""
                                            )
                                            if Colors:
                                                print(
                                                    colorize(
                                                        "DB responded",
                                                        Colors.RESPONSE + Colors.BOLD,
                                                    )
                ...
                if message.content and (
                    "query" in (message.content or "").lower()
                    or "call" in (message.content or "").lower()
                ):
