...
class OpenPipeClient:
    ...
    def add_dataset_entries(self, dataset_id: str, entries: List[Dict[str, Any]]) -> None:
        ...
        response = requests.post(
            url, headers=self.headers, json={"datasetId": dataset_id, "entries": entries}
        )
        response.raise_for_status()

    def create_finetune(
        self, dataset_id: str, slug: str, base_model: str = "meta-llama/Llama-2-13b-hf"
    ) -> str:
        ...
        response = requests.post(
            ...
            json={"datasetId": dataset_id, "slug": slug, "baseModel": base_model},
        )
        response.raise_for_status()
    ...
    def get_finetune_status(self, finetune_id: str) -> Dict[str, Any]:
        ...
        response = requests.get(url, headers=self.headers, params={"fineTuneId": finetune_id})
        response.raise_for_status()
    ...
    for msg in conversation_history:
        ...
        if role == "assistant":
            if tool_calls:
                ...
                agent_msg = {"role": "assistant", "content": None, "tool_calls": tool_calls}
...
def main():
    ...
    add_entries_parser.add_argument(
        "--episodes-dir", type=str, required=True, help="Directory containing episode JSON files"
    )
    ...
    create_finetune_parser.add_argument(
        "--slug", type=str, required=True, help="Fine-tune slug/name"
    )
    create_finetune_parser.add_argument(
        "--base-model",
        type=str,
    ...
    if args.command == "create-dataset":
        ...
        finetune_id = client.create_finetune(args.dataset_id, args.slug, args.base_model)
